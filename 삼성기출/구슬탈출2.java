...
public class 구슬탈출2 {
		...
		for(int i = 0; i < N; i++) {
			...
			for(int j = 0; j < M; j++) {
				...
				if(map[i][j] == 'O') {
					...
					blue = new Ring(0, 0, i, j, 0);
				} else if(map[i][j] == 'R') {
					red = new Ring(i, j, 0, 0, 0);
				}
			}
		}
	...
	public static int bfs() {
		Queue<Ring> queue = new LinkedList<>();
		queue.add(new Ring(red.rx, red.ry, blue.bx, blue.by, 1));
		visited[red.rx][red.ry][blue.rx][blue.ry] = true;
		
		while(!queue.isEmpty()) {
			Ring marble = queue.poll();
			...
			for(int i = 0; i < 4; i++) {
				...
				if(!visited[newRx][newRy][newBx][newBy]) {
					...
					queue.add(new Ring(newRx, newRy, newBx, newBy, curCnt+1));
				}
			}
		}
...
class Ring {
	...
	Ring(int rx, int ry, int bx, int by, int cnt) {
		this.rx = rx;
		this.ry = ry;
		this.bx = bx;
		this.by = by;
